...
module.exports.set = function set(config) {
	...
	return Promise.all(['urg', 'fon', 'tec', 'rrf', 'stv', 'cd2', 'el']
	.map(ext => {
		return readFile(path.join(dir, `svxlink.cfg`), 'utf8').then((data) => {
			...
			return writeFile(path.join(dir, `svxlink.cfg`), ini.encode(current))
		})
	}))
	.then(() => {
	...
	.then(() => {
		return readFile(path.join(dir, 'svxlink.d', 'ModuleMetarInfo.conf'), 'utf8').then((data) => {
			const current = ini.decode(data)
			...
			return writeFile(path.join(dir, 'svxlink.d', 'ModuleMetarInfo.conf'), ini.encode(current))
		})
	})

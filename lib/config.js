	...
	return readFile(path.join(dir, `svxlink.cfg`), 'utf8')
		...
		.then(() => {
			return readFile(
				...
				// SA818 - s'assurer que la section "[SA818] existe dans svxlink.cfg "
		...
		.then(() => {
			return readFile(
				...
				const current = ini.decode(data)
				const tabwifi = Object.keys(current)
				const wifiindex = tabwifi.indexOf("wifi-security")
				// wifi
				//current.wifi.ssid = `${wifi_ssid}`


				//à remettre :
				//const {wifidir} = '/etc/NetworkManager/system-connections/'
				//current.wifi.ssid = `${wifi_ssid}`
				//current.wifi-security.psk = `${wpa_key}`
				
				
				current.wifi.ssid = `${tabwifi}`
				//current[wifiindex[psk]] = `${wpa_key}`

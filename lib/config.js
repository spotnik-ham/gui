...
module.exports.set = function set(config) {
	config.callsign = config.callsign.toUpperCase()
	config.band_type = config.band_type.toUpperCase()
	config.departement = config.Departement.toUpperCase()
	const callsign = config.callsign
	const band_type = config.band_type
	const departement = config.Departement
	...
	return readFile(path.join(dir, `svxlink.cfg`), 'utf8')
		...
		.then(() => {
			if ((spotVersion === '4') || fs.existsSync('/etc/spotnik/svxlink.fdv')) {
				return readFile(path.join(dir, `svxlink.fdv`), 'utf8')
					.then(data => {
						...
						current.SimplexLogic.CALLSIGN = callsign
						if (current.ReflectorLogic) {
							current.ReflectorLogic.CALLSIGN = `(${departement}) ${callsign} DV`
						}

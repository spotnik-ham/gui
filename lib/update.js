...
async function versionguigit() {
	return await fetch('https://api.github.com/repos/spotnik-ham/gui/contents/version_gui?ref=Version_4').then(res => {
		if (!res.ok) {
			throw res
		}
		let data = await res.json()
		//let data2 = res.json().content
		//		let buff = new Buffer(data, 'base64');
		//		let text = buff.toString('ascii');
		console.log('versionguigit ==========================')
		console.log('res : ', res)
		console.log('data : ', await data)
		console.log('data.toString(ascii)', await data.toString('ascii'))

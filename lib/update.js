...
function majFTP() {
	chmod('/tmp/MAJ/test.sh', 0o755)
	//const ws = new Writable();
	var maj = spawn('/tmp/MAJ/test.sh')

	//console.log('majFTP spawn :*: ', maj)
	sseUpd.send(`majFTP spawn :*: `) //${JSON.stringify(maj)}`)
	maj.stdout.on('data', (data) => {
		sseUpd.send(data.toString('utf8'));
		console.log("maj.stdout ******* : ", data.toString('utf8'))
	});
